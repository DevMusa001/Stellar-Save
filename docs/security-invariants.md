...
| 12 | `Group::advance_cycle` cannot silently wrap the `current_cycle` counter; overflow panics rather than producing an incorrect cycle number | `group.rs` (`advance_cycle`, `checked_add`) | `advance_cycle_panics_when_group_complete`, `advance_cycle_does_not_wrap_u32` |
| 13 | `raise_dispute` vote-count increment and majority-threshold calculation both use checked arithmetic; an impossibly large `member_count` returns `Err(Overflow)` rather than producing a wrong threshold | `contract.rs` (`raise_dispute`) | `raise_dispute_threshold_no_overflow_for_max_half`, `raise_dispute_vote_count_checked_add_overflows_at_max` |
| 14 | `PoolInfo::completion_percentage` uses checked arithmetic for the intermediate `u64` multiplication; near-overflow inputs saturate to a valid 0–100 range without panicking | `pool.rs` (`completion_percentage`) | `completion_percentage_saturates_safely_at_max_contributors`, `completion_percentage_zero_member_count` |
| 15 | `Group::add_member` cannot silently wrap `member_count`; the overflow panics. In practice `member_count < MAX_MEMBERS` is enforced upstream by `GroupFull`, making the panic unreachable via normal operation | `group.rs` (`add_member`, `checked_add`) | `add_member_panics_on_u32_overflow` |
| 16 | `join_group` increments `member_count` with `checked_add`; the `GroupFull` guard fires first in all reachable paths, making the overflow branch unreachable under normal operation | `contract.rs` (`join_group`) | `join_group_group_full_fires_before_overflow` |
| 17 | `remove_member` decrements `member_count` with `checked_sub`; attempting to decrement below zero returns `Err(Overflow)` | `contract.rs` (`remove_member`) | (guarded by `NotMember` check; structural invariant — member existence verified before decrement) |
